"""
I am new to machine learning and really excited for all the new techniques I am learning.
Any feedback is welcome.

This is my very first cut of the solution to the titanic problem on kaggle without any feature
engineering. I will be working on improving this further. I had a 76.08% accuracy score on
...
def main():
    ...
    # Following code was used to find the optimal parameter for knn with the optimal features
    # results = {}
    ...
    # feature_col = ['Pclass', 'Sex', 'Embarked'], k = 42 has the highest accuracy at 0.811

    # Following code was used to find the optimal features for logistics regression
    # results = {}
    # all_features = ['Pclass', 'Sex', 'Age', 'SibSp', 'Parch', 'Fare', 'Embarked']
    # for L in range(0, len(all_features) + 1):
    #     for feature_col in itertools.combinations(all_features, L):
    ...
    #         results[feature_col] = cross_val_score(LogisticRegression(),
    #                                                X, y, cv=10,
    #                                                scoring='accuracy').mean()

    # Log reg with features ('Pclass', 'Sex', 'Age', 'SibSp', 'Embarked')
    # has an accuracy of 0.813
    # which means logreg is a better fit than k = 42 with pclass, sex and embarked

    feature_col = ['Pclass', 'Sex', 'Age', 'SibSp', 'Embarked']
    #  These features were determined using the 10 fold cross validation method
    # as described above

    X = data[feature_col]
    y = data[target_col]
    log = logreg_prediciting(X, y)

    data = pd.read_csv('test.csv', index_col=0)  # Testing data
    data = transform(data)

    testing_response = log.predict(data[feature_col])

    predicted = pd.DataFrame(testing_response, index=data.index, columns=['Survived'])
    predicted.to_csv('gender_submission.csv')


def k_nearest(X, y, k):
    # Three conditions for KNN
    # 1. X and y should have numeric data

    if type(X) is pd.DataFrame:
        first_feature_value = X.iloc[0][0]
        last_feature_value = X.iloc[-1][-1]
        first_target_value = y.iloc[0]
        last_target_value = y.iloc[-1]
    else:
        first_feature_value = X[0, 0]
        last_feature_value = X[-1, -1]
        first_target_value = y[0]
        last_target_value = y[-1]

    allowed_types = [np.float64, np.float32, np.int64, np.int32, int, float]
    assert type(first_feature_value) in allowed_types
    assert type(last_feature_value) in allowed_types
    assert type(first_target_value) in allowed_types
    assert type(last_target_value) in allowed_types

    # 2. Features and response should be numpy arrays or dataframes
    assert type(X) in [np.ndarray, pd.DataFrame]
    assert type(y) in [np.ndarray, pd.Series]

    # 3. Features and response should have specific shapes
